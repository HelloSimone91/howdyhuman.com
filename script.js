...
// Debug status message
        ...
        // Values data will be loaded from JSON
        let values = [];
        ...
        // Language state (global for loadValuesAndInitialize)
        let currentLanguage = 'en';

        // Function to fetch, parse, and initialize data
        async function loadValuesAndInitialize() {
            try {
                showStatus("Loading values...");
                const fileName = (currentLanguage === 'es') ? 'values-es.json' : 'values-en.json';
                const response = await fetch(fileName);
                if (!response.ok) {
                    throw new Error(`HTTP error ${response.status} while fetching ${fileName}`);
                }
                values = await response.json();
                showStatus("Values loaded successfully.");

                // Now that 'values' is populated, proceed with initialization
                initializeMainAppLogic();

            } catch (error) {
                console.error("Error loading or initializing values:", error);
        ...
        // Main app logic moved into this function
        function initializeMainAppLogic() {
            try {
                showStatus("App initializing with loaded data...");
                ...
                // Initialize the app (using the now populated 'values' array)
                initializeValuesDictionary();

                // Update values count
                updateValuesCount(); // Ensure this uses the global 'values'

                showStatus("App initialized successfully with dynamic data!");
            } catch (error) {
                console.error("Error in initializeMainAppLogic:", error);
                showStatus(`Error initializing app logic: ${error.message}`, true);
                fallbackInitialization(); // Attempt fallback
            }
        }

        // Wait for DOM to be fully loaded then load values and initialize
        document.addEventListener('DOMContentLoaded', function() {
            loadValuesAndInitialize();
        });
        ...
        function translateToSpanish() {
            ...
            showStatus("Spanish translation would appear here. This feature is coming soon!");
        }
